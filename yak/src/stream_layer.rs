...
///
/// Stream IDs are `u32`. Byte sizes/offsets are `u64`.
pub trait StreamLayer: Send + Sync {
    ...
    fn create_stream(&self, compressed: bool) -> Result<u32, YakError>;
    ...
    /// Get the `AdminSlotId` for the `index`-th upper layer section.
    ///
    /// Index 0 = first upper section (L4), etc.
    /// Implemented as `self.l2.admin_slot_for_upper(index + 1)` for the real L3.
