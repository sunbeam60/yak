...
/// Close an SFS file.
pub fn close_sfs(sfs: Sfs) -> CliResult {
    sfs.close()
        .map_err(|e| CliError::new(format!("Error closing SFS file: {}", e)))
}

/// Open a stream, read its entire contents, close the stream.
pub fn read_stream_fully(sfs: &Sfs, stream_path: &str) -> Result<Vec<u8>, CliError> {
    let handle = sfs
