...
class TestLocking:
    ...
    def test_multiple_readers(self, fs):
        ...
        h1 = fs.open_stream("data.bin", sfs.OpenMode.READ)
        h2 = fs.open_stream("data.bin", sfs.OpenMode.READ)
    ...
    def test_reader_blocks_writer(self, fs):
        ...
        # Open for reading
        h_read = fs.open_stream("data.bin", sfs.OpenMode.READ)

        # Try to open for writing - should fail
        with pytest.raises(sfs.SfsError):
            fs.open_stream("data.bin", sfs.OpenMode.WRITE)
    ...
    def test_writer_blocks_reader(self, fs):
        ...
        # Open for writing
        h_write = fs.open_stream("data.bin", sfs.OpenMode.WRITE)

        # Try to open for reading - should fail
        with pytest.raises(sfs.SfsError):
            fs.open_stream("data.bin", sfs.OpenMode.READ)
    ...
    def test_writer_blocks_writer(self, fs):
        ...
        # Open for writing
        h1 = fs.open_stream("data.bin", sfs.OpenMode.WRITE)
