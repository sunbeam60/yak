...
class TestVerifyClean:
    """Verify returns empty list on valid SFS files."""

    def test_empty_sfs(self, tmp_path):
        """A freshly created SFS (just root dir stream) is clean."""
    ...
    def test_with_streams(self, tmp_path):
        """SFS with dirs and data streams verifies clean."""
        sfs_path = str(tmp_path / "test.sfs")
        f = sfs.Sfs.create(sfs_path)
        f.mkdir("docs")
        f.mkdir("docs/sub")
    ...
    def test_varying_block_params(self, tmp_path, biw, bss):
        ...
        sfs_path = str(tmp_path / f"test_{biw}_{bss}.sfs")
        f = sfs.Sfs.create(sfs_path, block_index_width=biw, block_size_shift=bss)
    ...
    def test_verify_in_write_mode(self, tmp_path):
        """Verify works when SFS is opened in write mode too."""
        sfs_path = str(tmp_path / "test.sfs")
        f = sfs.Sfs.create(sfs_path)
...
class TestVerifyCorruption:
    ...
    def test_corrupt_file_truncated(self, tmp_path):
        ...
        original_size = os.path.getsize(sfs_path)
