...
use crate::helpers::{close_sfs, expect_args, expect_args_range, open_sfs};
...
pub fn cmd_ls(args: &[String]) -> CliResult {
    ...
    expect_args_range(rest, 1, 2, "sfs ls [-r] <sfs-file> [dir]")?;

    let sfs = open_sfs(&rest[0], OpenMode::Read)?;
...
fn ls_flat(sfs: &Sfs, dir: &str) -> CliResult {
    let entries = sfs
        .list(dir)
...
fn ls_recursive(sfs: &Sfs, dir: &str, display_prefix: &str) -> CliResult {
    let entries = sfs
        .list(dir)
        .map_err(|e| CliError::new(format!("Error listing directory: {}", e)))?;
...
pub fn cmd_mkdir(args: &[String]) -> CliResult {
    expect_args(args, 2, "sfs mkdir <sfs-file> <dir>")?;
...
pub fn cmd_rmdir(args: &[String]) -> CliResult {
    expect_args(args, 2, "sfs rmdir <sfs-file> <dir>")?;
...
pub fn cmd_mv_dir(args: &[String]) -> CliResult {
    expect_args(args, 3, "sfs mv-dir <sfs-file> <old> <new>")?;
