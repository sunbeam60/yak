...
/// available block ID. A `header` file stores the SFS header with slot registry.
///
...
impl BlockLayer for BlocksFromFiles {
    ...
    fn open(path: &str, _mode: OpenMode) -> Result<Self, SfsError> {
        ...
        if slots.is_empty() {
            return Err(SfsError::IoError("no sections found in header".to_string()));
        }
        ...
        if l2_payload.len() < L2_MOCK_PAYLOAD_SIZE as usize {
            return Err(SfsError::IoError("L2 payload too short".to_string()));
        }
