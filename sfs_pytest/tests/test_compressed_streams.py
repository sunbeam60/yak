...
class TestCompressedLifecycle:
    """Create, open, close compressed SFS files."""

    def test_create_with_compression(self, tmp_path):
        """Create an SFS file with compression configured."""
        sfs_path = str(tmp_path / "test.sfs")
        fs = sfs.Sfs.create(sfs_path, BLOCK_INDEX_WIDTH, BLOCK_SIZE_SHIFT)
        fs.close()
        assert os.path.exists(sfs_path)

    def test_reopen_compressed_sfs(self, tmp_path):
        """Create compressed SFS, close, reopen."""
        sfs_path = str(tmp_path / "test.sfs")
        fs = sfs.Sfs.create(sfs_path, BLOCK_INDEX_WIDTH, BLOCK_SIZE_SHIFT)
        fs.close()
...
class TestCompressedStreamIO:
    ...
    def fs(self, tmp_path):
        sfs_path = str(tmp_path / "test.sfs")
        f = sfs.Sfs.create(sfs_path, BLOCK_INDEX_WIDTH, BLOCK_SIZE_SHIFT)
        yield f
        f.close()
...
class TestCompressedPersistence:
    ...
    def test_persistence_small(self, tmp_path):
        ...
        fs = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ)
...
class TestMixedStreams:
    """Compressed and uncompressed streams in the same SFS."""
    ...
    def test_mixed_create_and_read(self, tmp_path):
        ...
        h_plain = fs.open_stream("plain.bin", sfs.OpenMode.READ)
        assert fs.read(h_plain, len(plain_data)) == plain_data
        fs.close_stream(h_plain)

        h_comp = fs.open_stream("comp.bin", sfs.OpenMode.READ)
...
class TestCompressibility:
    ...
    def test_compressible_data_smaller(self, tmp_path):
        ...
        fs = sfs.Sfs.create(plain_path, BLOCK_INDEX_WIDTH, BLOCK_SIZE_SHIFT)
    ...
    def test_incompressible_data_still_correct(self, tmp_path):
        ...
        fs = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ)
        handle = fs.open_stream("random.bin", sfs.OpenMode.READ)
        result = fs.read(handle, len(data))
        assert result == data
        fs.close_stream(handle)
        fs.close()
...
class TestCompressedReadOnly:
    ...
    def test_write_fails_readonly(self, tmp_path):
        ...
        fs = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ)
        h = fs.open_stream("data.bin", sfs.OpenMode.READ)
