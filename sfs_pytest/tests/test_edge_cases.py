...
class TestEdgeCases:
    ...
    def test_deeply_nested(self, fs):
        ...
        assert entries[0].entry_type == sfs.EntryType.STREAM
    ...
    def test_stream_names_with_spaces(self, fs):
        ...
        # Reopen and read
        h_read = fs.open_stream("my file.txt", sfs.OpenMode.READ)
    ...
    def test_reopen_after_close(self, fs):
        ...
        # Reopen for reading
        h_read = fs.open_stream("persistent.dat", sfs.OpenMode.READ)
        ...
        h_write = fs.open_stream("persistent.dat", sfs.OpenMode.WRITE)
        fs.seek(h_write, 15)  # Seek to end
        fs.write(h_write, b" more")
        fs.close_stream(h_write)

        # Reopen again and verify
        h_read2 = fs.open_stream("persistent.dat", sfs.OpenMode.READ)
