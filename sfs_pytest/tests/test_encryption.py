...
class TestEncryptionBasic:
    ...
    def test_create_encrypted(self, tmp_path):
        """Create an encrypted SFS file, write data, close, reopen with
        correct password, and read back the data."""
        sfs_path = str(tmp_path / "enc.sfs")
        password = b"test-password-123"

        # Create and write
        f = sfs.Sfs.create(sfs_path, password=password)
        ...
        f = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ, password=password)
        sh = f.open_stream("hello.txt", sfs.OpenMode.READ)
        data = f.read(sh, 100)
    ...
    def test_wrong_password(self, tmp_path):
        """Reopening an encrypted file with the wrong password raises SfsError."""
        sfs_path = str(tmp_path / "enc.sfs")
        f = sfs.Sfs.create(sfs_path, password=b"correct")
        f.close()

        with pytest.raises(sfs.SfsError, match="wrong password"):
            sfs.Sfs.open(sfs_path, sfs.OpenMode.READ, password=b"incorrect")

    def test_no_password_on_encrypted(self, tmp_path):
        """Reopening an encrypted file without a password raises SfsError."""
        sfs_path = str(tmp_path / "enc.sfs")
        f = sfs.Sfs.create(sfs_path, password=b"secret")
        f.close()

        with pytest.raises(sfs.SfsError, match="encryption required"):
    ...
    def test_unencrypted_unchanged(self, tmp_path):
        ...
        sfs_path = str(tmp_path / "plain.sfs")
        ...
        sh = f.open_stream("test.txt", sfs.OpenMode.READ)
        data = f.read(sh, 100)
    ...
    def test_is_encrypted(self, tmp_path):
        ...
        unencrypted files."""
        enc_path = str(tmp_path / "enc.sfs")
        plain_path = str(tmp_path / "plain.sfs")

        f = sfs.Sfs.create(enc_path, password=b"secret")
        assert f.is_encrypted() is True
        f.close()

        f = sfs.Sfs.create(plain_path)
        assert f.is_encrypted() is False
        f.close()

        # Verify is_encrypted persists across reopen
        f = sfs.Sfs.open(enc_path, sfs.OpenMode.READ, password=b"secret")
        assert f.is_encrypted() is True
        f.close()

        f = sfs.Sfs.open(plain_path, sfs.OpenMode.READ)
        assert f.is_encrypted() is False
        f.close()
...
class TestEncryptionIntegrity:
    ...
    def test_encrypted_verify(self, tmp_path):
        ...
        sfs_path = str(tmp_path / "enc.sfs")
        f = sfs.Sfs.create(sfs_path, password=b"verify-pw")
        f.mkdir("docs")
        sh = f.create_stream("hello.txt")
        f.write(sh, b"Hello!")
        f.close_stream(sh)
        ...
        f = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ, password=b"verify-pw")
        issues = f.verify()
        f.close()
    ...
    def test_encrypted_verify_after_delete(self, tmp_path):
        ...
        sfs_path = str(tmp_path / "enc.sfs")
        f = sfs.Sfs.create(sfs_path, password=b"del-pw")
        ...
        f = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ, password=b"del-pw")
        issues = f.verify()
        f.close()
        assert issues == [], f"Expected no issues, got: {issues}"
    ...
    def test_encrypted_persistence(self, tmp_path):
        ...
        f = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ, password=password)
        sh = f.open_stream("data.bin", sfs.OpenMode.READ)
...
class TestEncryptionDirectories:
    ...
    def test_encrypted_directories(self, tmp_path):
        ...
        sh = f.open_stream("alpha/beta/deep.txt", sfs.OpenMode.READ)
        data = f.read(sh, 100)
    ...
    def test_encrypted_rename(self, tmp_path):
        ...
        f = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ, password=password)
        sh = f.open_stream("renamed.txt", sfs.OpenMode.READ)
        data = f.read(sh, 100)
...
class TestEncryptionMultiblock:
    ...
    def test_encrypted_multiblock(self, tmp_path):
        ...
        f = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ, password=password)
        sh = f.open_stream("big.bin", sfs.OpenMode.READ)
        result = f.read(sh, len(data))
        assert result == data
        f.close_stream(sh)
        f.close()
    ...
    def test_encrypted_small_blocks(self, tmp_path):
        ...
        sfs_path = str(tmp_path / "enc_small.sfs")
        password = b"small-pw"

        # bss=4 → 16-byte blocks (minimum for XTS), biw=2
        f = sfs.Sfs.create(
            sfs_path, block_index_width=2, block_size_shift=4, password=password
        )
        ...
        f = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ, password=password)
        sh = f.open_stream("tiny.bin", sfs.OpenMode.READ)
        result = f.read(sh, len(data))
        assert result == data
        f.close_stream(sh)
        f.close()
    ...
    def test_encrypted_varying_params(self, tmp_path, biw, bss):
        ...
        sfs_path = str(tmp_path / f"enc_{biw}_{bss}.sfs")
        password = b"param-pw"
        data = b"X" * 500
...
class TestEncryptionCompression:
    ...
    def test_encrypted_compression(self, tmp_path):
        ...
        sfs_path = str(tmp_path / "enc_comp.sfs")
        password = b"comp-pw"
        ...
        # Reopen and verify
        f = sfs.Sfs.open(sfs_path, sfs.OpenMode.READ, password=password)
        sh = f.open_stream("compressed.bin", sfs.OpenMode.READ)
        result = f.read(sh, len(data))
        assert result == data
        f.close_stream(sh)
