...
fn derive_key_material(
    ...
    t_cost: u32,
    p_cost: u32,
) -> Result<[u8; KDF_OUTPUT_LEN], SfsError> {
    let params = argon2::Params::new(m_cost, t_cost, p_cost, Some(KDF_OUTPUT_LEN))
        .map_err(|e| SfsError::IoError(format!("argon2id params: {}", e)))?;
    let argon2 = Argon2::new(argon2::Algorithm::Argon2id, argon2::Version::V0x13, params);

    let mut output = [0u8; KDF_OUTPUT_LEN];
    argon2
        .hash_password_into(password, salt, &mut output)
        .map_err(|e| SfsError::IoError(format!("argon2id derivation failed: {}", e)))?;
    Ok(output)
}
...
pub(crate) fn create_encryption(
    ...
    kek.wrap(&master_key, &mut wrapped_key)
        .map_err(|e| SfsError::IoError(format!("AES-KW wrap failed: {:?}", e)))?;
...
pub(crate) fn open_encryption(
    ...
    if computed_hash != config.verification_hash {
        return Err(SfsError::WrongPassword(
    ...
    kek.unwrap(&config.wrapped_key, &mut master_key)
        .map_err(|_| SfsError::WrongPassword("AES-KW unwrap failed".to_string()))?;
...
pub(crate) fn deserialize_config(data: &[u8]) -> Result<EncryptionConfig, SfsError> {
